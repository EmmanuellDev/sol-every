...
const Notification = ({ type, message, description, txid, onHide }) => {
  ...
  const getIcon = () => {
    switch (type) {
      case "success":
        return <CheckCircleIcon className="text-success mr-1 h-8 w-8" />;
      case "info":
        return <InformationCircleIcon className="text-info mr-1 h-8 w-8" />;
      case "error":
        return <XCircleIcon className="text-error mr-1 h-8 w-8" />;
      default:
        return null;
    }
  };
  ...
  <div className="p-4">
    <div className="flex items-center">
      ...
      <div className="ml-2 w-0 flex-1">
        ...
        { description ? (
          <p className="text-fgd-1 mt-0.5 text-sm">{description}</p>
        ) : null}
        { txid ? (
